    ...
    while not game_over:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_SPACE and spin_active:
                    wheel.stop_spin()

        if handler.data['score'] >= next_spin_score and not spin_active and WHEEL_TIME < 3 :
            wheel.start_spin()
            spin_active = True
            next_spin_score += 300  # Set the next milestone
            WHEEL_TIME += 1
        ...
        if spin_active:
            ...
            if not wheel.spinning:
                spin_active = False
                cloud.replace_current_fruit(wheel.get_current_fruit())
        ...
        # pygame.draw.line(screen, (255, 255, 255), (355, WHITE_LINE_Y), (config.screen.width, WHITE_LINE_Y), 2)
